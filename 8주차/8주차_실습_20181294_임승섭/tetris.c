...
void DrawBlockWithFeatures(int y, int x, int blockID, int blockRotate) {
	...
	DrawRecommend(recBlockY, recBlockX, blockID, recBlockR); // Draw recommend position
}
...
int recommend(RecNode *root){
	...
	int max_level = VISIBLE_BLOCKS;
	// rotate의 개수를 나타내는 array
	int rot[7] = {2, 4, 4, 4, 1, 2, 2};
	int r = rot[nextBlock[root->level]];
	...
	for (int i = 0; i < r; i++) {
		for (int j = 0; j < WIDTH; j++) {
			if (CheckToMove(root->recField, nextBlock[root->level], i, 1, j) == 0)
				continue;

			int k = 0;
			while (CheckToMove(root->recField, nextBlock[root->level], i, k+1, j) != 0)
				k++;
			...
			for (int p = 0; p < HEIGHT; p++) {
				for (int q = 0; q < WIDTH; q++) {
					child->recField[p][q] = root->recField[p][q];
				}
			}
			c_score += AddBlockToField(child->recField, nextBlock[root->level], i, k, j);
			c_score += DeleteLine(child->recField);

			//child acc_score
			child->acc_score = c_score;

			//child_level
			child->level = root->level + 1;

			if (child->level < max_level) {
				c_score += recommend(child);
			}
